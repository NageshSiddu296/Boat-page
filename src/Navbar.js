import { StrictMode } from "react";
import { createRoot } from "react-dom/client";
import "./styles.css";

export default function Navbar() {
  return (
    <>
      <div className="top-bar">
        <h1 className="top-text">
          Get the binge on with Aavante bar tune. Grab now @3999!
        </h1>
      </div>
      <div className="Navbar">
        <div>
          <a href="https://www.googleadservices.com/pagead/aclk?sa=L&ai=DChcSEwi9qYLKgr79AhX5nUsFHcQMD70YABAAGgJzZg&ohost=www.google.com&cid=CAESauD2eMMv9hqJvMrrA3QUqXzm3rzWoBXsLaD1KsM3f2qs-ExjsG1thC2OP6jITnOtXbLxEuzxXNcZO4ZNhF1B_ptl86wtpfdMaawdKMhCuxUfH7fEfutCqkVhGQnA9wDAfVuFawS-zWWziPA&sig=AOD64_0aCW6cjycWLxTns529macnC_Xxhw&q&adurl&ved=2ahUKEwjBxvzJgr79AhXzg2MGHfHUD7wQ0Qx6BAgJEAE">
            <img
              className="logo"
              src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/boat_logo_small.webp?v=1672379935"
              alt=""
            />
          </a>
        </div>
        <div className="drop-downs">
          <div className="drop-downs-button">
            <button
              class="btn btn-secondary dropdown-toggle"
              type="button"
              data-bs-toggle="dropdown"
              aria-expanded="false"
            >
              CATEGORIES
            </button>
            <ul class="dropdown-menu">
              <li>
                <a class="dropdown-item" href="#">
                  Action
                </a>
              </li>
              <li>
                <a class="dropdown-item" href="#">
                  Another action
                </a>
              </li>
              <li>
                <a class="dropdown-item" href="#">
                  Something else here
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div className="drop-downs">
          <button className="nav-buttons">DAILY DEALS</button>
        </div>
        <div className="drop-downs">
          <button className="nav-buttons">GIFT WITH BOAT</button>
        </div>
        <div className="drop-downs">
          <button
            class="btn btn-secondary dropdown-toggle"
            type="button"
            data-bs-toggle="dropdown"
            aria-expanded="false"
          >
            MORE
          </button>
          <ul class="dropdown-menu">
            <li>
              <a class="dropdown-item" href="#">
                Action
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                Another action
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                Something else here
              </a>
            </li>
          </ul>
        </div>
        <div className="search-box">
          <input className="input" placeholder="search"></input>
          <img
            className="login-logo"
            src="data:image/png;base64,"
            alt=""
          />
          <img
            className="cart-logo"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Cart_7e68f099-c0bf-4966-a9ef-14d1a3af07ad.png?v=1673260730"
            alt=""
          />
        </div>
      </div>
    </>
  );
}
